...
function recruitListGet(){
  ...
  $page = $page == 1 ? $page-1 : $page *10 - 9; //データベースから取得するときの開始行数を指定　ページが1のときはそのまま　それ以外の場合は10倍して-9
  $conn = getConnection();

  if($word !== ""){//↓みづらい　いずれも部分一致検索
    if($how == "m_content"){//内容（紹介文？）を使った検索
      $data = $conn -> query("SELECT * from matching WHERE ".$how." LIKE '%".$word."%' and a_host_id != ".$a_id." order by ".$sort." desc limit ".$page." ,10 ");
    }else if ($how == "m_tag"){//タグを利用した検索
      $data = $conn -> query("SELECT * from matching m left join skill_tag s on m.m_id = s.m_id
                          WHERE skill LIKE '%".$word."%' and a_host_id != ".$a_id." order by ".$sort." desc limit ".$page." ,10");
    }
  }else{
    $data = $conn -> query('select * from matching where a_host_id != '.$a_id.' order by m_id desc limit '.$page.',10');
  }
  $conn = null;
  return $data;
}
...
function recGet(){
  ...
  $data = $conn -> query('select * from matching m left join profile p on m.a_host_id = p.a_id where m_id = '.$m_id);
  $data = $data -> fetch();
  $conn = null;
  return $data;
}
...
function recruitAppList(){
  ...
  $data = $conn ->prepare('select m.m_title ,ac.a_name
  from app_waiting a left join matching m on a.m_id = m.m_id left join account ac on a.a_id = ac.a_id
  where a.a_id = :a_id ');
...
function getIcon($id){
  $conn = getConnection();
  $data = $conn -> query('select * from profile where a_id = '.$id);
  $data = $data -> fetch();
  return $data;
}
...
function getTag($m_id){
    $conn = getConnection();
    $data = $conn->prepare('select * from skill_tag where m_id = :m_id ');
