  ...
  */
  ...
  for($i=0; $i<count($talk);$i++){
    ...
    if($whichmyid["a_host_id"] == $a_id){//相手の情報をとるため逆になる
      $opid = $whichmyid["a_app_id"];

    }else{
      $opid = $whichmyid["a_host_id"];

  }
    $data = $conn->query('select t_id,(select m_title from matching m where m.m_id = t.m_id) as m_title,
    (select p_nickname from profile where a_id = '.$opid.') as a_name,
    (select p_icon from profile where a_id = '.$opid.') as p_icon  from talkroom t  where t_id = '.$talk[$i]["t_id"]);
    // $data = $conn -> query('select * from talkroom tr left join talk t on tr.t_id = t.t_id
